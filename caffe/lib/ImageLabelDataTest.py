...
class ImageLabelDataTest(caffe.Layer):
    def setup(self, bottom, top):
        ...
        if self.train_mode == 0:
            for line in self.indices:
                ...
                print '###############################################################'
                print '/RGB/' + impath + '.jpg'
                self.imglist.append('/RGB/' + impath + '.jpg')
                self.depthlist.append('/depth/' + impath + '.jpg')
    ...
    def load_gt16(self, idx):
        ...
        self.gt4 = cv2.resize(im,
                              (np.int(np.floor(self.gt2.shape[1] / 2.0)), np.int(np.floor(self.gt2.shape[0] / 2.0))))
        self.gt8 = cv2.resize(im,
                              (np.int(np.floor(self.gt4.shape[1] / 2.0)), np.int(np.floor(self.gt4.shape[0] / 2.0))))
        self.gt16 = cv2.resize(im,
                               (np.int(np.floor(self.gt8.shape[1] / 2.0)), np.int(np.floor(self.gt8.shape[0] / 2.0))))
        self.gt32 = cv2.resize(im,
                               (np.int(np.floor(self.gt16.shape[1] / 2.0)), np.int(np.floor(self.gt16.shape[0] / 2.0))))
